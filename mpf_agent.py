...
# Stores the last email generated by tool_build_weekly_email so run_agent can send it
_last_email_result: dict | None = None
...
def tool_get_portfolio_state(tool_input: dict) -> str:
    ...
    return json.dumps({
        ...
        "portfolio_weighted_1yr_return_pct": round(weighted_return, 2),
        "holdings": _PORTFOLIO,
    })
...
MPF Portfolio Intelligence System""".strip()
    ...
    global _last_email_result
    _last_email_result = result
...
def run_monthly_job() -> None:
    ...
    Full monthly workflow:
      1. Send fund-sheet update request email.
      2. Run the MPF agent (recalculates performance + builds email).
      3. Send the performance result email.
    """
    today = date.today()
    ...
    # Step 2 & 3: run agent and send its result email
...
"""
    ...
    while True:
        ...
        if response.stop_reason != "tool_use":
            ...
            if send_result and _last_email_result:
                email_text = _last_email_result["email"]
...
if __name__ == "__main__":
    ...
    if cmd == "--scheduler":
        ...
        # Run the full monthly job once right now
        run_monthly_job()
    else:
